...
void UIObject::updateCycle(bool updateAnimator) {
    for (auto &i: children) {
        i->transformedPosition = transformedPosition + i->position + innerTransform;
        ...
        i->position.animate();
        if (i->useFbo) {
            if ((i->myFbo.getWidth() != i->size.x) ||
                (i->myFbo.getHeight() != i->size.y)) {
                i->setFboSize(ofPoint(ofGetWidth(), ofGetHeight()));
            }
        }
